...
class Results:
    ...
    def save_results(self, args):
        for i in range(self.model_num):
            tem = getattr(args, 'tem', 0.5)
            lr_w = getattr(args, 'lr_w', 1.0)
            with open(f"{args.dataset}_tao{tem}_model{i}.txt", 'a') as f:
                f.write(f"τ={tem}, γ={args.alpha}, lr_w={lr_w}\n")
        ...
        csv_path = os.path.join(raw_dir, 'all_seed_results.csv')
        file_exists = os.path.exists(csv_path)
        ablation_variant = getattr(args, 'ablation_variant', None)
        model_name = 'FairMoE' if str(self.model).lower() == 'fairmoe' else str(self.model)
        category = 'fairmoe' if model_name == 'FairMoE' else 'fair_baseline'
        note_parts = []
        if ablation_variant:
            note_parts.append(f'ablation_variant={ablation_variant}')
        if getattr(args, 'auto_profile', False):
            note_parts.append('auto_profile=True')

        fieldnames = [
            'dataset', 'model', 'category', 'seed', 'auc', 'f1', 'acc', 'dp', 'eo',
            'best_epoch', 'threshold', 'hidden', 'source', 'log_path', 'note',
        ]
        with open(csv_path, 'a', newline='', encoding='utf-8') as f:
            ...
            for seed in range(self.seed_num):
                writer.writerow({
                    ...
                    'category': category,
                    'seed': seed,
                    'auc': float(self.auc[seed, 0]),
                    'f1': float(self.f1[seed, 0]),
                    'acc': float(self.acc[seed, 0]),
                    'dp': float(self.parity[seed, 0]),
                    'eo': float(self.equality[seed, 0]),
                    'best_epoch': '',
                    'threshold': '',
                    'hidden': getattr(args, 'hidden', ''),
                    'source': 'self-run',
                    'log_path': '',
