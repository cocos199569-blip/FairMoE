...
# ==========================
# build k-hop adjacency
# ==========================
def build_k_hop_sparse(adj: SparseTensor, k: int):
    out = adj
    for _ in range(k - 1):
        out = out @ adj
    out = out.remove_diag()
...
class FairDataset:
    ...
    def adj2edge_index(adj, fea_num):
        ...
        if not sp.isspmatrix_coo(adj):
            adj = adj.tocoo()
        row = torch.tensor(adj.row.tolist(), dtype=torch.long)
        col = torch.tensor(adj.col.tolist(), dtype=torch.long)
    ...
    def load_data(self, sens_attr=None, label_number=None, split_ratio=None, val_idx=True, sens_number=None):
        ...
        if self.dataset == 'credit':
            ...
            adj, features, labels, sens, idx_train, idx_val, idx_test = load_credit(dataset, sens_attr,
                                                                                    predict_attr, path_credit,
        ...
        # ==========================
        # Build multi-scale adjacency
        # ==========================
        num_nodes = features.size(0)

        adj_1 = self.adj2sparse(adj, num_nodes).to(self.device)
        adj_2 = build_k_hop_sparse(adj_1, 2).to(self.device)
        ...
        # --- 图结构 ---
        self.edge_index = edge_index  # COO edge_index，给 GAT / bias 流用
        self.edge_index_spar = edge_index_spar  # SparseTensor（备用，不强制用）

        self.adj_1 = adj_1  # 1-hop，多尺度公平流
        self.adj_2 = adj_2  # 2-hop，多尺度公平流
