#在ms_2bias3_newtest4.py的基础上微调
#之前：
#2026.3.7上午开始测试bail数据集
#2026.3.14: 在 gate 损失中加入可微 DP/EO 项（小权重起步）。在验证选模里加公平约束（不是只看 F1/ROC）。
# 26.3.16 开始测试credit数据集
#26.3.17 试着改成数据特征自适应选模
#26.3.25 不想调了，最后就这样吧，如果需要就调一调Pokec数据集好了
#26.3.27  再调一调bail数据集上的效果吧
"""
2026.3.28:
试 fairness refinement stage
Stage 1：正常训练
用你现在的模型训练到收敛
拿到高 AUC 的 backbone / expert 表征
Stage 2：公平微调
冻结 backbone
冻结 accuracy expert
只训练：
...
"""同时使用weight1、2，不做softmax处理
使用能使 F1-score 最大化的最佳阈值，"""

"""在German数据集上
python train_test.py --dataset german --dropout 0.1 --lr 0.007114466864080137 --alpha 0.13 --beta 0.68 --min_fair 0.26 --warmup_steps 300 --gate_fair_weight 2.4 --res_val_f1_weight 0.5 --res_val_roc_weight 0.5 --seed_num 5 --epochs 1000
⭐结果稳定是：
AUCROC: 70.63 ± 3.9
F1-score: 82.41 ± 0.12
ACC: 70.16 ± 0.32
...
在（/data/runcode/xxy/moe_ms4/gpuvenv）这个环境上的结果：
AUCROC: 71.28 ± 1.68
F1-score: 82.41 ± 0.34
ACC: 70.72 ± 0.89
Parity: 5.82 ± 5.26
...
"""在bail数据集上
python train_test.py --dataset bail --dropout 0.1 --lr 0.007114466864080137 --alpha 0.13 --beta 1.6 --min_fair 0.7 --warmup_steps 100 --gate_fair_weight 6 --res_val_f1_weight 0.5 --res_val_roc_weight 0.5 --seed_num 5 --epochs 1000 --min_auc 0.89 --min_f1 0.79 --max_parity 2.3 --max_equality 2.3 --constraint_mode hard --constraint_penalty_weight 8.0
结果是：
AUCROC: 90.91 ± 1.17
F1-score: 81.23 ± 1.4
ACC: 86.29 ± 1.05
...
python train.py --dataset bail --dropout 0.27 --lr 0.0023585982559417577 --alpha 0.23 --beta 0.42 --min_fair 0.74 --warmup_steps 200 --gate_fair_weight 8.9 --res_val_f1_weight 0.7 --res_val_roc_weight 0.3 --min_auc 89 --min_f1 80 --max_parity 2.5 --max_equality 2.5 --constraint_mode soft --constraint_penalty_weight 10 --use_group_fair_loss --lambda_dp 2.5 --lambda_eo 2.5 --seed_num 5 --epochs 1000
AUCROC: 90.29 ± 1.1
F1-score: 79.24 ± 1.52
ACC: 84.68 ± 0.77
Parity: 4.14 ± 1.2
...
python train.py --dataset bail --seed_num 5 --epochs 1000 --auto_profile --auto_profile_verbose
AUCROC: 90.47 ± 1.12
F1-score: 79.58 ± 1.53
ACC: 84.84 ± 1.24
Parity: 4.75 ± 0.93
...
现在 在（/data/runcode/xxy/moe_ms4/gpuvenv）这个环境上
⭐CUDA_VISIBLE_DEVICES=1 python train.py --dataset bail --seed_num 5 --epochs 1000 --auto_profile --auto_profile_verbose
AUCROC: 90.12 ± 1.02
F1-score: 79.05 ± 0.91
ACC: 84.67 ± 0.74
Parity: 3.6 ± 1.8
...
python train.py --dataset credit --seed_num 5 --epochs 1000 --auto_profile --auto_profile_verbose
AUCROC: 72.44 ± 0.27
F1-score: 87.54 ± 0.12
ACC: 78.04 ± 0.14
Parity: 0.19 ± 0.26
...
⭐在（/data/runcode/xxy/moe_ms4/gpuvenv）这个环境上
AUCROC: 72.44 ± 0.27
F1-score: 87.53 ± 0.12
ACC: 78.03 ± 0.13
Parity: 0.16 ± 0.21
